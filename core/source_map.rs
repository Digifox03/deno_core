...
pub struct SourceMapContainer<T>
where
  T: SourceMapGetter,
{
  getter: T,
  ...
  T: SourceMapGetter,
{
  ...
  pub fn apply_source_map(
    ...
    let maybe_source_map_entry = self.maps.get(file_name);
    ...
    match maybe_source_map {
      Cow::Borrowed(_) => {}
      Cow::Owned(maybe_source_map) => {
        self.maps.insert(file_name.to_owned(), maybe_source_map);
      }
    }
